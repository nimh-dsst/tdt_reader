# Modified by Joshua Lawrimore on 2025-04-09
#
...
def snip_maker(data_snip):
    ...
    for ind in range(len(gap_points)):
        if nchan == 1:
            ...
            snip_store.append(
                data_snip.data[gp_index + chan_index, :].flatten()
            )
        else:
            ...
            for chan in range(nchan):
                ...
                if len(chan_index) != nchunks:
                    warnings.warn(
                        "channel {0} was shortened to {1} chunks".format(
                            chan, nchunks
                        ),
                        Warning,
                        stacklevel=2,
...
def parse_tbk(tbk_path):
    ...
    try:
        ...
        delimInd = [
            m.start() for m in re.finditer(r"\[USERNOTEDELIMITER\]", s)
        ]
...
def read_sev(
    ...
    *,
    ...
    if outdir is not None and export is None:
        warnings.warn(
            "no export is selected, outdir parameter is ignored",
            Warning,
            stacklevel=2,
        )
    ...
    if os.path.isfile(sev_dir):
        ...
        if just_names == 0:
            ...
            if n_txtfiles < 1 and verbose:
                ...
                start_search = re.compile(
                    r"recording started at sample: (\d*)"
                )
                ...
                for txt_path in txt_file_list:
                    ...
                    temp_sample_info["start_sample"] = int(
                        temp_start_sample[0]
                    )
                    ...
                    if len(gap_text):
                        temp_sample_info["gaps"] = np.array(
                            gap_text, dtype=np.int64
                        )
                        temp_sample_info["gap_text"] = "\n   ".join(
    ...
    if fs > 0 and verbose:
        print(
            "Using {:.4f} Hz as SEV sampling rate for {}".format(
                fs, event_name
            )
        )
    ...
    for file in file_list:
        ...
        with open(file["fullname"], "rb") as sev:
            ...
            stream_header.size_bytes = np.fromfile(
                sev, dtype=np.uint64, count=1
            )[0]
            ...
            stream_header.file_version = np.fromfile(
                sev, dtype=np.uint8, count=1
            )[0]
            ...
            if stream_header.file_version < 4:
                ...
                stream_header.decimate = np.fromfile(
                    sev, dtype=np.uint8, count=1
                )[0]
                stream_header.rate = np.fromfile(
                    sev, dtype=np.uint16, count=1
                )[0]
            else:
                raise Exception(
            ...
            if stream_header.file_version > 0:
                stream_header.fs = (
                    np.power(2.0, (np.float64(stream_header.rate) - 12))
                    * 25000000
...
upgrade to OpenEx v2.18 or above\n""".format(
            ...
            file["itemsize"] = np.uint64(
                np.dtype(stream_header.data_format).itemsize
            )
    ...
    for this_event in event_names:
        ...
        for ch in channels:
            try:
                ...
                warnings.warn(
                    "channel {0} not found in {1} event".format(
                        ch, temp_event_name
                    ),
                    Warning,
                    stacklevel=2,
        ...
        for jj in range(num_ranges):
            ...
            for jjj in hour_values:
                ...
                if curr_samples + npts[jjj] >= absolute_end_sample[jj]:
                    end_hour_samples_end[jj] = (
                        absolute_end_sample[jj] - curr_samples
                    )
                ...
                if export in ["binary"]:
                    ...
                    if prefix:
                        ...
                        outname = os.path.join(
                            outdir,
                            "{0}{1}.{2}".format(
                                temp_outfile, temp_event_name, ftype
                            ),
                        )
            ...
            if export in ["csv", "interlaced"]:
                for kk in range(
                    ...
                    for chan in channels:
                        ...
                        # open file
                        with open(
                            file_list_temp[file_num]["fullname"], "rb"
                        ) as f:
                    ...
                    if scale != 1:
                        temp_data = (
                            np.float64(temp_data) * np.float64(scale)
                        ).astype(data_format)
                    if export in ["interlaced"]:
                        temp_data.astype(export_type, copy=False).T.tofile(
                            file_handle
                        )
                    elif export in ["csv"]:
                        np.savetxt(
                    ...
                    for kk in range(
                        ...
                        with open(
                            ...
                            if export in ["binary"]:
                                if scale != 1:
                                    ddd = (
                                        np.float64(ddd) * np.float64(scale)
                                    ).astype(data_format)
...
def read_block_with_ts(
    ...
    *,
    ...
    data = read_block_with_ts(block_path), where block_path is a string, retrieves
    ...
    optional keyword arguments:
        ...
        evtype      array of strings, specifies what type of data stores to
                      ...
                      example:
                          data = read_block_with_ts(block_path, evtype=['epocs','snips'])
                              > returns only epocs and snips
        ranges      array of valid time range column vectors.
                      example:
                          tr = np.array([[1,3],[2,4]])
                          data = read_block_with_ts(block_path, ranges=tr)
        ...
        headers     var, set to 1 to return only the headers for this
                      ...
                      example:
                        heads = read_block_with_ts(block_path, headers=1)
                        data = read_block_with_ts(block_path, headers=heads, evtype=['snips'])
                        data = read_block_with_ts(block_path, headers=heads, evtype=['streams'])
        combine     list, specify one or more data stores that were saved
                      ...
                      example:
                        data = read_block_with_ts(block_path, combine=['StS1'])
        export      string, choose a data exporting format.
                        csv:        data export to comma-separated value files
                                    streams: one file per store, one channel per column
                                    epocs: one column onsets, one column offsets
    ...
    if outfile is not None and export is None:
        warnings.warn(
            "no export is selected, outfile parameter is ignored",
            Warning,
            stacklevel=2,
        )
    ...
    if not use_outside_headers:
        ...
        try:
            ...
            raise Exception(
                "tsq file {0} could not be opened".format(tsq_list[0])
            )
    ...
    if not do_headers_only:
        try:
            ...
            raise Exception(
                "tev file {0} could not be opened".format(header.tev_path)
            )
            ...
            for sort_id in os.listdir(sort_path):
                if os.path.isdir(os.path.join(sort_path, sort_id)):
                    ...
                    sort_files = tdt.get_files(
                        os.path.join(sort_path, sort_id),
                        ".SortResult",
                        ignore_mac=True,
                    )
            ...
            if sortname in sort_ids["sort_id"]:
                for i in range(len(sort_ids["sort_id"])):
                    if sort_ids["sort_id"][i] == sortname:
                        ...
                        ddd = np.fromfile(
                            sort_ids["fileNames"][i], dtype=np.uint8
                        )
            ...
            for target in targets:
                ...
                if len(note_line) >= eee + 2:
                    if note_line.startswith(test_str):
                        setattr(
                            data.info, target.lower(), note_line[eee + 1 :]
                        )
                        target_found = True
                        break
            if target_found:
                ...
                if note_id == "none" or no_buttons:
                    ...
                    if "date changed to" in this_note_text:
                        ...
                        temp = datetime.strptime(
                            this_note_text[16:].lower(), yearfmt
                        )
                        curr_day = temp.day
    ...
    if not use_outside_headers:
        ...
        while True:
            ...
            if rem != 0:
                warnings.warn(
                    "Block did not end cleanly, removing last {0} headers".format(
                        rem
                    ),
                    Warning,
                    stacklevel=2,
                ...
                store_codes.append(
                    {
                        ...
                        "type_str": code_to_type(
                            heads[1, unique_ind[counter]]
                        ),
                        "ucf": check_ucf(heads[1, unique_ind[counter]]),
                        "epoc_type": epoc_to_type(
                            heads[1, unique_ind[counter]]
                        ),
            ...
            for store_code in store_codes:
                if store_code["code"] in [
                    tdt.EVMARK_STARTBLOCK,
                    tdt.EVMARK_STOPBLOCK,
                ]:
                    continue
                ...
                store_code["var_name"] = tdt.fix_var_name(
                    store_code["name"], verbose
                )
                ...
                if store_code["type_str"] == "epocs":
                    buddy = "".join(
                        [
                            str(chr(c))
                            for c in np.array([store_code["buddy"]]).view(
                                np.uint8
                            )
                        ]
                    )
                    ...
                    if store_code["type_str"] != "epocs":
                        ...
                        if header.stores[var_name].type_str != "scalars":
                            header.stores[var_name].fs = np.double(
                                np.array([store_code["temp"][9]]).view(
                                    np.float32
                                )[0]
                            )
                ...
                if len(note_str) > 0 and store_code["type_str"] in [
                    "scalars",
                    "epocs",
                ]:
                ...
                if store_code["type_str"] != "epocs":
                    ...
                    if store_code["type_str"] == "snips":
                        ...
                        if (
                            len(custom_sort_codes) > 0
                            and var_name in custom_sort_event
                        ):
                            # apply custom sort codes
                            for tempp in range(len(custom_sort_event)):
                                if (
                                    type(custom_sort_codes[tempp])
                                    == np.ndarray
        ...
        print(
            "read from t={0}s to t={1}s".format(
                np.round(t1, 2), np.round(lastRead, 2)
            )
        )
        ...
        if len(note_text) > 0 and "Note" not in epocs.name:
            ...
            epocs.code.append(
                np.array([ord(x) for x in "Note"], dtype=np.uint8).view(
                    np.uint32
                )[0]
            )
        ...
        for ii in range(len(epocs.name)):
            if epocs.type[ii] == "offset":
                ...
                if var_name not in header.stores.keys():
                    warnings.warn(
                        epocs.buddies[ii] + " buddy epoc not found, skipping",
                        Warning,
                    )
                    continue
                ...
                # fix time ranges
                if (
                    header.stores[var_name].offset[0]
                    < header.stores[var_name].onset[0]
                ):
                            ...
                            if "|" in head_name:
                                ...
                                if primary in header.stores.keys():
                                    if verbose:
                                        print(
                                            current_name,
                                            "is secondary epoc of",
                                            primary,
                                        )
                                    header.stores[
                                        var_name
                                    ].offset = header.stores[primary].offset
        ...
        for var_name in header.stores.keys():
            ...
            if header.stores[var_name].type_str in ["streams", "snips"]:
                if "data" in header.stores[var_name].keys():
                    header.stores[var_name].data = header.stores[
                        var_name
                    ].data.view(np.uint64)
            if "chan" in header.stores[var_name].keys():
    ...
    for var_name in header.stores.keys():
        ...
        if use_outside_headers:
            data[current_type_str][var_name] = copy.deepcopy(
                header.stores[var_name]
            )
        else:
        ...
        if "ts" in header.stores[var_name].keys():
                ...
                if len(filter_ind[jj]) > 0:
                    ...
                    if current_type_str == "streams":
                        ...
                        if not bSkip:
                            ...
                            data[current_type_str][var_name].start_time[jj] = (
                                temp[0]
                            )
                    else:
                        data[current_type_str][var_name].filtered_ts[jj] = (
                            data[current_type_str][var_name].ts[filter_ind[jj]]
                        )
                    ...
                    if hasattr(data[current_type_str][var_name], "data"):
                        data[current_type_str][var_name].filtered_data[jj] = (
            ...
            if current_type_str == "streams":
                ...
                if hasattr(data[current_type_str][var_name], "chan"):
                    if hasattr(
                        ...
                        if current_type_str == "snips":
                            if (
                                len(set(data[current_type_str][var_name].chan))
                                == 1
                            ):
                                data[current_type_str][var_name].chan = [
                ...
                if hasattr(data[current_type_str][var_name], "sortcode"):
                    if hasattr(
                        ...
                        data[current_type_str][
                            var_name
                        ].sortcode = np.concatenate(
                            data[current_type_str][var_name].filtered_sort_code
                        )
                        delattr(
                            data[current_type_str][var_name],
                            "filtered_sort_code",
                        )
                    else:
                        data[current_type_str][var_name].sortcode = []
            ...
            if len(filter_ind) > 0:
                data[current_type_str][var_name].onset = data[
                    current_type_str
                ][var_name].onset[filter_ind]
                data[current_type_str][var_name].data = data[current_type_str][
                    var_name
                ].data[filter_ind]
                data[current_type_str][var_name].offset = data[
                    current_type_str
                ][var_name].offset[filter_ind]
                if var_name == "Note":
                    try:
                        data[current_type_str][var_name].notes = note_text[
                            filter_ind
                        ]
                    except:
                        warnings.warn(
                            "Problem with Notes.txt file",
                            Warning,
                            stacklevel=2,
                        )
        ...
        if hasattr(header.stores, var_name):
            try:
                ...
                data[current_type_str][var_name].notes.index = note_index[
                    valid_ind
                ]
        ...
        if current_type_str == "epocs":
            if export in ["csv"]:
                if prefix:
                    outname = os.path.join(
                        outdir,
                        "{0}_{1}.{2}".format(prefix, current_name, export),
                    )
                else:
                    outname = os.path.join(
                        outdir, "{0}.{1}".format(current_name, export)
                    )
                with open(outname, "w") as f:
                    ...
                    for i, _ in enumerate(xx.data):
                        f.write(
                            "\n%.15f,%.15f,%.15f"
                            % (xx.data[i], xx.onset[i], xx.offset[i])
                        )
                        if current_name == "Note":
            ...
            if nchan > 1:
                ...
                for xx in range(nchan):
                    ind.append(
                        np.where(
                            data[current_type_str][current_name].chan == xx + 1
                        )[0]
                    )
                ...
                data[current_type_str][current_name].ts = data[
                    current_type_str
                ][current_name].ts[ind[0]]
            ...
            if 0 in channels:
                ...
                use_all_known = True
            channels = sorted(list(set(channels)))
            use_all_known = len(all_ch.intersection(set(channels))) == len(
                all_ch
            )
            ...
            if not use_all_known:
                ...
                if len(valid_ind) == 0:
                    raise Exception(
                        "channels {0} not found".format(repr(channels))
                    )
                if not nodata:
                    all_offsets = data[current_type_str][current_name].data[
                        valid_ind
                    ]
                data[current_type_str][current_name].chan = data[
                    current_type_str
                ][current_name].chan[valid_ind]
                data[current_type_str][current_name].sortcode = data[
                    current_type_str
                ][current_name].sortcode[valid_ind]
                data[current_type_str][current_name].ts = data[
                    current_type_str
                ][current_name].ts[valid_ind]
            else:
                if not nodata:
                    all_offsets = data[current_type_str][current_name].data
            ...
            if not nodata:
                ...
                for f in range(len(arr)):
                    ...
                    data[current_type_str][current_name].data.append(
                        tev_data[ind.flatten().astype(np.uint64)].reshape(
                            (-1, npts)
                        )
                    )
                    event_count += len(ind)
                ...
                if len(data[current_type_str][current_name].chan) > 1:
                    data[current_type_str][current_name].chan = data[
                        current_type_str
                    ][current_name].chan[:totalEvents]
                data[current_type_str][current_name].sortcode = data[
                    current_type_str
                ][current_name].sortcode[:totalEvents]
                data[current_type_str][current_name].ts = data[
                    current_type_str
                ][current_name].ts[:totalEvents]
            ...
            if current_name in sev_names:
                ...
                if d is not None:  # exporter returns None
                    ...
                    if expected_fs > 0 and (
                        np.abs(expected_fs - detected_fs) > 1
                    ):
                        warnings.warn(
                ...
                for jj in range(num_ranges):
                    ...
                    if len(fc) == 1:
                        ...
                        valid_ind = [
                            i for i, x in enumerate(fc) if x in channels
                        ]
                        fc = fc[valid_ind]
                        nchan = np.uint64(len(list(set(fc))))

                    chan_index = np.zeros(nchan, dtype=np.uint64)
                    these_offsets = data[current_type_str][
                        current_name
                    ].filtered_data[jj][valid_ind]
                    ...
                    if export in ["csv", "interlaced", "binary"]:
                        ddd = np.zeros(
                            [nchan, npts], dtype=current_data_format
                        )
                    else:
                    ...
                    if export in ["csv", "interlaced", "binary"]:
                        print(
                            "exporting TEV stream {0}...".format(current_name)
                        )
                    ...
                    while (
                        ...
                        iter = max(iter // 2, 1)
                        markers = these_offsets[
                            range(0, len(these_offsets), iter)
                        ]
                        ...
                        if export in ["binary"]:
                            ...
                            if prefix:
                                ...
                                outname = os.path.join(
                                    outdir,
                                    "{0}{1}.{2}".format(
                                        temp_outfile, current_name, ftype
                                    ),
                                )
                            file_handle = open(outname, "w")
                            if export == "csv":
                                file_handle.write(
                                    ",".join(
                                        [str(x) for x in sorted(list(set(fc)))]
                                    )
                                    + "\n"
                                )
                    ...
                    for f in range(len(arr)):
                        ...
                        if f == len(arr) - 1:
                            read_size = (
                                these_offsets[-1] - markers[f]
                            ) // sz + npts
                        else:
                            read_size = (
                                markers[f + 1] - markers[f]
                            ) // sz + npts
                        ...
                        # convert offsets from bytes to indices in data array
                        relative_offsets = ((xxx - min(xxx)) / sz)[
                            np.newaxis
                        ].T
                        ind = relative_offsets + np.tile(
                            range(npts), [len(relative_offsets), 1]
                        ...
                        for kk in range(len(relative_offsets)):
                            ...
                            if not np.any(ind[kk, :] <= len(tev_data)):
                                if not found_empty:
                                    warnings.warn(
                                        "Data missing from tev file for store:{0} channel:{1} time:{2}s".format(
                                            current_name,
                                            fc[channel_offset],
                                            np.round(
                                                t1
                            ...
                            if export in ["csv", "interlaced", "binary"]:
                                ...
                                if ready_for_export[arr_index] == True:
                                    warnings.warn(
                                        "This channel already ready? tev file out of order?",
                                        Warning,
                                    )
                                ddd[arr_index, :] = tev_data[
                                    ind[kk, :].flatten().astype(np.uint64)
                                ]
                                ...
                                if np.all(ready_for_export):
                                    ...
                                    if export == "csv":
                                        ...
                                        for x in sorted(file_handles.keys()):
                                            ...
                                            ddd[index, :].astype(
                                                export_type, copy=False
                                            ).T.tofile(file_handles[x])
                                    # reset ready indicators
                                    ready_for_export = [
                                        False for i in ready_for_export
                                    ]
                                    duration = time.time() - start_time
                                    sys.stdout.write(
                                        "\r...%d%%, %d seconds elapsed"
                    ...
                    if export in ["csv", "interlaced"]:
                        ...
                        td_time = time2sample(
                            data[current_type_str][current_name].start_time[
                                jj
                            ],
                            current_freq,
                            to_time=True,
                        ...
                        data[current_type_str][current_name].data[jj] = data[
                            current_type_str
                        ][current_name].data[jj][
                            :, minSample : int(maxSample + 1)
                        ]
            ...
            if export is None:
                if len(data[current_type_str][current_name].data) == 1:
                    ...
                    if len(data[current_type_str][current_name].data) > 0:
                        if (
                            data[current_type_str][current_name].data.shape[0]
                            == 1
                        ):
                            data[current_type_str][current_name].data = data[
                                current_type_str
            ...
            if store:
                if isinstance(store, str):
                    ...
                    if sev_name not in store:
                        continue
            if sev_name not in [
                data.streams[fff].name for fff in data.streams.keys()
            ]:
                d = read_sev(
                    block_path,
                    event_name=sev_name,
    ...
    if bitwise != "":
        if not (
            hasattr(data.epocs, bitwise) or hasattr(data.scalars, bitwise)
        ):
            raise Exception(
        ...
        for i in range(sz):
            xxx = np.array(
                [data[bitwisetype][bitwise].data[i]], dtype=np.uint32
            )
            ...
            for ind in changes:
                ...
                if curr_state[ind]:
                    ...
                    if hasattr(data[bitwisetype][bitwise].bitwise, ffield):
                        ...
                        data[bitwisetype][bitwise].bitwise[ffield] = (
                            tdt.StructType()
                        )
                        data[bitwisetype][bitwise].bitwise[
                            ffield
                        ].onset = np.array([big_array[0, i]])
                        data[bitwisetype][bitwise].bitwise[
                            ffield
                        ].offset = np.array([])
                else:
